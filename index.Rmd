...
- Apresenta uma medida de evidência para testar hipóteses estatísticas, denominada valor-s.
- Neste trabalho, vamos generalizar a definição do valor-s.
- Discute um teste baseado no valor-s com a possibilidade de indecisão.
...
- O termo "agnóstico" surge no cenário de testes de hipóteses.
- Definem algumas propriedades lógicas para testes agnósticos.
- O único teste fortemente coerente é o teste baseado em regiões de confiança.
...
`r Citet(myBib, 'izbicki2023rethinking')` propõem um procedimento para testar hipóteses baseado em regiões de confiança.
...
<div style="font-size: 50px; text-align: center;">
  <strong>Família Regular de Regiões de Confiança</strong>
</div>
...
- A região de confiança converge em probabilidade para o parâmetro $\boldsymbol{\theta}$.
...
- $\bigcup\limits_{\alpha \in (0,1)} \tilde{\Lambda}_\alpha^{\boldsymbol{x}_n} = \Theta$, para cada $\boldsymbol{x}_n\in\mathcal{X}$;

- $\bigcap\limits_{\alpha \in (0,1)} \tilde{\Lambda}_\alpha^{\boldsymbol{x}_n} \neq \varnothing$, para cada $\boldsymbol{x}_n\in\mathcal{X}$.
...
<div style="font-size: 70px; text-align: center;">
  <strong>Valor-S Generalizado</strong>
</div>
...
- O valor-s é o maior nível de significância tal que a região de confiança contém pelo menos um elemento em comum do conjunto $\overline{\Theta}_0$.

- $0<s_{\boldsymbol{x}_n}(\Theta_0)<1$ indica que há informação que contradiz parcialmente o conjunto $\Theta_0$ de modo que quanto mais próximo de 0, mais informações contrárias a $\Theta_0$ foram observadas.

```{r, echo = F, fig.align = 'center', out.width = '55%'}
knitr::include_graphics('Figuras/valors2_40.png')
```
...
$$
\begin{equation*}
    T_{\boldsymbol{\theta}}(\boldsymbol{d}_n) =
...
\end{equation*}
$$
em que $\boldsymbol{d}_n=[\mathbf{X}\ \mathbf{Y}]$ são os dados observados, $\hat{\boldsymbol{\theta}} = \left(\mathbf{X}^{\top}\mathbf{X}\right)^{-1}\mathbf{X}^{\top}\mathbf{Y}$ é o EMV de $\boldsymbol{\theta}$ e a matriz $\mathbf{X}^{\top}\mathbf{X}$ é positiva definida.

Para $\alpha\in(0,1)$, seja
$$
\begin{equation}
    \Lambda_{\alpha}^{\boldsymbol{d}_n} =
    \left\{\boldsymbol{\theta}\in\mathbb{R}^{p}:T_{\boldsymbol{\theta}}(\boldsymbol{d}_n) \leq F^{-1}(1-\alpha)\right\},
\end{equation}
$$
em que $F(\cdot)$ é a função distribuição F-Snedecor com $p$ e $n-p$ graus de liberdade.
...
\begin{align*}
    ...
    \sup\left\{\alpha\in(0,1):\alpha \leq 1 - F\left(\inf_{\boldsymbol{\theta}\in\Theta_0}T_{\boldsymbol{\theta}}(\boldsymbol{d}_n)\right)\right\} \\ &=
    1 - F\left(\inf_{\boldsymbol{\theta}\in\Theta_0}T_{\boldsymbol{\theta}}(\boldsymbol{d}_n)\right).
\end{align*}
$$

---

# Propriedades do Valor-S

**Propriedades**. Seja $s_{{\boldsymbol{x}_n}}$ o valor-s baseado em uma FRRC $\{\Lambda_{\alpha}^{{\boldsymbol{x}_n}}\}_{\alpha\in(0,1)}$ para $\boldsymbol{\theta}$. Então, para $\Theta_i \subseteq \Theta$, $i=0,1,2$, tem-se as seguintes propriedades:
...
Hipóteses $$\mathcal{H}_{1}:\theta_1=\theta_2=0 \quad \mbox{e} \quad \mathcal{H}_{2}:\theta_1=0.$$
...
<div style="font-size: 70px; text-align: center;">
  <strong>Conjuntos de Decisão</strong>
</div>
...
**Teorema.** Se $\Theta_0\in \mathcal{R}_{\alpha_1}^{\boldsymbol{x}_n}$, então
...
**Teorema.** Se $\Theta_0\in\mathcal{A}_{\alpha_2}^{\boldsymbol{x}_n}$, então
...
**Teorema.** Se $\Theta_0\in \mathcal{U}_{\alpha_1,\alpha_2}^{\boldsymbol{x}_n}$, então
...
**Teorema.** Sejam $\Theta_1\subseteq\Theta_2$ e $\alpha_1,\alpha_2\in(0,1)$.
...
**Teorema.** Sejam $\Theta_1,\Theta_2\subseteq\Theta$ e $\alpha_1,\alpha_2\in(0,1)$.
...
O teste baseado no valor-s satisfaz todas as propriedades lógicas desde que $\alpha_1=\alpha_2$.
...
<div style="font-size: 70px; text-align: center;">
  <strong>Controle dos Erros e Consistência do Teste</strong>
</div>

---

# Controle dos Erros Tipo I e II
...
- Probabilidade máxima de cometer o erro tipo I é menor que $\alpha_1$.
- Probabilidade máxima de cometer o erro tipo II é menor que $\alpha_2$.
...
- Assintoticamente, as probabilidades de decisões corretas, como rejeitar $\boldsymbol{\theta} \in \Theta_0$ quando falsa ou aceitar $\boldsymbol{\theta} \in \Theta_0$ quando verdadeira, convergem para 1.
- Evita problemas do tipo "rejeitar a hipótese sempre que $n$ aumenta".
...
<div style="font-size: 70px; text-align: center;">
  <strong>Estudo de Simulação</strong>
</div>
...
- $R = 10000$ simulações de Monte Carlo.  
...
**Figura.** *Heatmaps* das probabilidades de decisão, (a) Rejeição, (b) Aceitação e (c) Agnóstico, estimadas via $10000$ simulações de Monte Carlo para diferentes cenários sob $\mathcal{H}_0:\theta_1\geq0$ e diferentes tamanhos amostrais.

```{r, echo = F, out.width = '100%', fig.align = 'center'}
knitr::include_graphics('Figuras/Heatmaps/HeatmapH0.png')
```

- As probabilidades de aceitação convergem pra 1 com o aumento de $n$.
- Essa convergência é mais rápida em cenários distantes da borda da hipótese testada.
...
**Figura.** *Heatmaps* das probabilidades de decisão, (a) Rejeição, (b) Aceitação e (c) Agnóstico, estimadas via $10000$ simulações de Monte Carlo para diferentes cenários sob $\mathcal{H}_1:\theta_1<0$ e diferentes tamanhos amostrais.

```{r, echo = F, out.width = '100%', fig.align = 'center'}
knitr::include_graphics('Figuras/Heatmaps/HeatmapH1.png')
```

- As probabilidades de rejeição convergem pra 1 com o aumento de $n$.
- Essa convergência é mais rápida em cenários distantes da borda da hipótese testada.
...
- Estudar as propriedades do teste proposto quando o modelo está mal especificado (*model misspecification*).

- Desenvolver uma teoria para modelos de aprendizado de máquina, como modelos de linguagem.
...
library(qrcode)
QR = qr_code('https://github.com/Andrey-Sarmento/ThesisPresentation.git')
plot(QR)
```
